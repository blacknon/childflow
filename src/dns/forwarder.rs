...
pub(super) fn start(
    ...
    let mut joins = Vec::<JoinHandle<Result<()>>>::new();
...
pub(super) fn forward_udp_query(query: &[u8], upstream_addr: SocketAddr) -> Result<Vec<u8>> {
...
pub(super) fn relay_bidirectional(left: &mut TcpStream, right: &mut TcpStream) -> Result<()> {
