...
fn connect_upstream_proxy(
    ...
    let domain = match upstream_addr {
    ...
    socket.connect(&upstream_addr.into()).with_context(|| {
        format!(
...
fn resolve_proxy_server(server: &ProxyServer) -> Result<SocketAddr> {
...
fn connect_tls_proxy(
    ...
    let connector = builder
        .build()
        .context("failed to build TLS connector for upstream proxy")?;
    connector
