...
use super::super::types::{IcmpRelayOutcome, Icmpv6EchoRequest, Icmpv6RawRequest};
use super::frames;

pub(in crate::network::rootless_internal::icmp) fn spawn_icmpv6_echo_worker(
    event_tx: Sender<RemoteEvent>,
    request: Icmpv6EchoRequest,
) {
    thread::spawn(move || {
        let child_probe = frames::v6::build_child_icmpv6_echo_probe(&request);

        let result = relay::relay_icmpv6_echo(
            request.remote_ip,
            request.hop_limit,
            request.identifier,
            request.sequence,
        ...
        .and_then(|outcome| match outcome {
                    IcmpRelayOutcome::Message(reply) => {
                        frames::v6::build_icmpv6_echo_reply_frame(&request, &reply)
                    }
                    IcmpRelayOutcome::Error {
                        ...
                        frames::v6::build_icmpv6_error_reply_frame(
...
pub(in crate::network::rootless_internal::icmp) fn spawn_icmpv6_raw_worker(
    event_tx: Sender<RemoteEvent>,
    request: Icmpv6RawRequest,
) {
    thread::spawn(move || {
        let child_probe = frames::v6::build_child_icmpv6_raw_probe(&request);

        let result =
            relay::relay_icmpv6_message(request.remote_ip, request.hop_limit, &request.message)
                .and_then(|outcome| match outcome {
                    IcmpRelayOutcome::Message(reply) => {
                        frames::v6::build_icmpv6_raw_reply_frame(&request, &reply)
                    }
                    IcmpRelayOutcome::Error {
                        source_ip,
                        icmp_type,
