...
if curl --connect-timeout 3 -fsS http://origin-http.demo:8080/ >/dev/null 2>&1; then
  echo "direct HTTP access unexpectedly succeeded" >&2
  exit 1
fi

if curl --connect-timeout 3 -kfsS https://origin-https.demo:8443/ >/dev/null 2>&1; then
...
if ./target/debug/childflow \
  ...
  -- \
  curl --connect-timeout 5 -fsS http://origin-http.demo:8080/ >/dev/null 2>&1; then
...
if ./target/debug/childflow \
  ...
  --proxy-password demo \
  -- \
  curl --connect-timeout 5 -kfsS https://origin-https.demo:8443/ >/dev/null 2>&1; then
...
./target/debug/childflow \
  ...
  --proxy-password demo \
  -- \
  curl --connect-timeout 5 -fsS http://origin-http.demo:8080/ >"$http_proxy_output"
...
./target/debug/childflow \
  ...
  --proxy-insecure \
  -- \
  curl --connect-timeout 5 -kfsS https://origin-https.demo:8443/ >"$https_proxy_output"
