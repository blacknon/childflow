...
`CARGO_TARGET_DIR` is set to `/tmp/childflow-target` so containerized builds do not depend on bind-mounted workspace ownership.
That target directory's `debug` and `release` subdirectories are also added to `PATH`, so after a build you can invoke `childflow` directly inside the container.
...
Or, after `cargo build`, run the built binary directly:
